...
def doChecksumize(romBytes: bytes, kindName: str | None, update: bool, outputPath: Path) -> int:
    if kindName is None:
...
def processArguments(args: argparse.Namespace):
    ...
    exit(doChecksumize(romBytes, kindName, update, outputPath))
...
def addSubparser(subparser: argparse._SubParsersAction[argparse.ArgumentParser]):
    ...
    parser.add_argument("-k", "-c", "--kind", "--cic", help="Used this variant to calculate the checksum instead of automatically detecting which kind the ROM uses", dest="kind", choices=ipl3checksum.CICKind.validNames())
